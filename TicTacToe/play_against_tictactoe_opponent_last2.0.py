    ...
    def step(self, action):
        ...
        if not self.action_defs[action + 1] in board.history_coords:
            ...
            if len(board.history) != 9 and self.is_terminal()[0] is False:
                ...
                opponent = None
            if self.action_defs[action + 1] == opponent:
                board.update_board(opponent, not self.player)
                board.render()

            if not self.action_defs[action + 1] in board.history_coords:
                if opponent not in board.history_coords and opponent is not None:
                    board.update_board(opponent, not self.player)
                    board.render()
...
model = PPO.load('./train/going_last/1.0/best_model_overall')
